...
class Base(TableBase):
    ...
    def from_line(self, line: str) -> "Base":
        ...
        self.approach_id = extract_field(line, w_bas.approach_id)
        self.surface_id = extract_field(line, w_bas.surface_id)
        self.ops_type = extract_field(line, w_bas.ops_type)
    ...
    def get_leading_dict(self) -> dict:
        return {
            ...
            "approach_id": clean_value(self.approach_id),
            "surface_id": clean_value(self.surface_id),
            "ops_type": clean_value(self.ops_type),
        }
