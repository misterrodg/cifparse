...
class Continuation(Base):
    ...
    def from_line(self, line: str) -> "Continuation":
        ...
        self.baz_lat = convert_lat_dms(extract_field(line, w_con.baz_lat))
        self.baz_lon = convert_lon_dms(extract_field(line, w_con.baz_lon))
        self.true, self.baz_mag_bearing = convert_bearing(
            extract_field(line, w_con.baz_mag_bearing)
        )
        self.dp_lat = convert_lat_dms(extract_field(line, w_con.dp_lat))
        self.dp_lon = convert_lon_dms(extract_field(line, w_con.dp_lon))
        self.baz_dist = convert_distance(extract_field(line, w_con.baz_dist))
        self.plus_minus = extract_field(line, w_con.plus_minus)
        self.pro_right = convert_int(extract_field(line, w_con.pro_right))
        self.pro_left = convert_int(extract_field(line, w_con.pro_left))
        self.cov_right = convert_int(extract_field(line, w_con.cov_right))
        self.cov_left = convert_int(extract_field(line, w_con.cov_left))
        _, self.baz_true_bearing = convert_bearing(
            extract_field(line, w_con.baz_true_bearing)
        )
        self.baz_source = extract_field(line, w_con.baz_source)
        _, self.az_true_bearing = convert_bearing(
            extract_field(line, w_con.az_true_bearing)
        )
